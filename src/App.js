...
import "./App.scss";
import PagerSound from "./assets/Fire_pager.mp3";
...
const App = () => {
  ...
  const DISABLED_RESET = buttonsDisabledState
    ? null
    : () => dispatch(handleReset());

  const BORDER_CLR =
    sessionStatusState === "session"
      ? { borderColor: "#388E3C" }
      : { borderColor: "#D32F2F" };
  const SESS =
    sessionStatusState === "session" ? "timer-numbers" : "timer-numbers-break";
  const BRK =
    sessionStatusState === "session" ? "timer-break-hidden" : "timer-break";
  ...
  return (
    <div id="main-container">
      ...
      <audio id="beep" src={PagerSound}></audio>
    </div>
  );
};
